...
# -------------------------
# LINK NORMALIZATION (v3.4.5)
# -------------------------
def normalize_link(link: str) -> str:
    """
    Normaliza links para reduzir falhas de extractor (principalmente TikTok shortlinks).
    - remove espaços/zero-width
    - remove / no final (especialmente vt.tiktok.com/.../)
    """
    if not link:
        return link
    ...
    # Normalização especial: TikTok shortlinks falham às vezes com "/" final
    low = link.lower()
    if "vt.tiktok.com/" in low or "vm.tiktok.com/" in low:
        link = link.rstrip("/")  # remove apenas trailing slashes
    return link
...
# -------------------------
def detect_source_from_link(link: str) -> str:
...
def ytdlp_print(url: str, template: str) -> str:
    cmd = [sys.executable, "-m", "yt_dlp"]
    if COOKIES.exists():
        cmd += ["--cookies", str(COOKIES)]
    cmd += ["--print", template, url]
    out = subprocess.check_output(cmd, text=True, encoding="utf-8", errors="ignore").strip()
    return out

def ytdlp_resolve_webpage_url(url: str) -> str | None:
    try:
        cmd = [sys.executable, "-m", "yt_dlp"]
        if COOKIES.exists():
            cmd += ["--cookies", str(COOKIES)]
        cmd += ["--print", "%(webpage_url)s", url]
        out = subprocess.check_output(cmd, text=True, encoding="utf-8", errors="ignore").strip()
...
# -------------------------
def social_get_title(url: str, fallback_prefix: str) -> str:
    try:
        out = ytdlp_print(url, "%(title)s")
        return out if out else f"{fallback_prefix}_{uuid.uuid4().hex[:6]}"
    except Exception:
        return f"{fallback_prefix}_{uuid.uuid4().hex[:6]}"

def social_download_audio_wav(url: str, out_dir: Path, prefix: str) -> Path:
    out_dir.mkdir(parents=True, exist_ok=True)
    out_template = str(out_dir / f"{prefix}_%(id)s.%(ext)s")
    ...
    wavs = sorted(out_dir.glob(f"{prefix}_*.wav"), key=lambda p: p.stat().st_mtime, reverse=True)
    if not wavs:
        raise RuntimeError("Não consegui gerar .wav via yt-dlp (talvez precise de cookies).")
    return wavs[0]


# -------------------------
# Drive (public link) via gdown
# -------------------------
...
def main():
    ...
    parser.add_argument("--link", type=str, help="Link universal (YouTube/Drive/Instagram/TikTok)")
    parser.add_argument("--accurate", action="store_true", help="Modo mais fiel (mais lento)")
    args = parser.parse_args()

    accurate = args.accurate

    # Normalize link inputs
    if args.link:
        args.link = normalize_link(args.link)
    if args.url:
        args.url = normalize_link(args.url)
    if args.drive:
        args.drive = normalize_link(args.drive)

    # Link universal
    if args.link:
        source = detect_source_from_link(args.link)

        if source == "youtube":
            args.url = args.link

        elif source == "drive":
            args.drive = args.link

        elif source == "instagram":
            title = social_get_title(args.link, "instagram")
            print(f"\n🌐 INSTAGRAM detectado: {title}")
            print("🎯 Modo:", "ACCURATE (mais fiel)" if accurate else "FAST (rápido)")

            wav = social_download_audio_wav(args.link, TEMP, prefix="ig")
            process_job("instagram", title, wav, {"url": args.link}, accurate)
            return

        elif source == "tiktok":
            title = f"tiktok_{uuid.uuid4().hex[:6]}"
            print(f"\n🌐 TIKTOK detectado: {title}")
            print("🎯 Modo:", "ACCURATE (mais fiel)" if accurate else "FAST (rápido)")

            url_try = args.link
            low = (args.link or "").lower()
            is_short = ("vt.tiktok.com" in low) or ("vm.tiktok.com" in low)

            def _try_download(u: str) -> Path:
                return social_download_audio_wav(u, TEMP, prefix="tt")

            try:
                wav = _try_download(url_try)
            except Exception as e1:
                if is_short:
                    print("🧭 TikTok shortlink falhou. Tentando resolver para URL final...")
                    resolved = ytdlp_resolve_webpage_url(args.link)
                    if resolved:
                        resolved = normalize_link(resolved)
                    if resolved and resolved != args.link:
                        print(f"🔗 Resolvido: {resolved}")
                        try:
                            wav = _try_download(resolved)
                            process_job("tiktok", title, wav, {"url": resolved, "original_url": args.link}, accurate)
                            return
                ...
                print("\n❌ Falha ao baixar TikTok.")
                print("➡️ Isso geralmente acontece quando:")
                print("   - o vídeo exige login, e seus cookies não estão válidos/atualizados")
                print("   - o vídeo é de conta privada ou com restrição regional")
                print("   - o extractor do TikTok está instável para esse vídeo (acontece)")
                print("\n✅ Tenta assim:")
                print("   1) Abra o link no Chrome (logado).")
                print("   2) Exporte cookies.txt de novo (na página do vídeo).")
                print(f"   3) Salve em: {COOKIES}")
                print("   4) Rode: pip install -U yt-dlp")
                print("\nDetalhe do erro:", e)
                return

            process_job("tiktok", title, wav, {"url": args.link}, accurate)
            return

        else:
            print("❌ Não reconheci o link.")
            return
    ...
    if args.url:
        ...
        process_job("youtube", title, wav, {"url": args.url}, accurate)
        return
    ...
    if args.drive:
        ...
        process_job("drive", title, wav, {"drive_url": args.drive, "downloaded_file": downloaded.name}, accurate)
        return
    ...
    if not videos:
        ...
        print("➡️ Use --link / --url / --drive OU coloque vídeos na pasta entrada.")
        return
    ...
    for video in videos:
        ...
        try:
            ...
            process_job("upload", video.stem, wav, {"file_name": video.name}, accurate)
        except Exception as e:
